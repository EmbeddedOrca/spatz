...
def ascending_data_generator(shape, prec):
    if prec == 64:
        return torch.arange(0, shape[0] + 1, dtype=torch.float64) % 4, {}
    elif prec == 32:
        return torch.arange(0, shape[0] + 1, dtype=torch.float32) % 4, {}
    elif prec == 16:
        return torch.arange(0, shape[0] + 1, dtype=torch.float16) % 4, {}
    elif prec == 8:
        return torch.arange(0, shape[0] + 1, dtype=torch.uint8) % 4, {}
