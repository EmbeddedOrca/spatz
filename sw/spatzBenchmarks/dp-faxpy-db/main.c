...
int main() {
  ...
  if (cid == 0) {
    ...
    // Set the memory to zero for testing
    snrt_memset(x, 0, dim * sizeof(double));
    snrt_memset(y, 0, dim * sizeof(double));
    snrt_memset(a, 0, sizeof(double));
  }
  ...
  if (cid == 0) {
    *a = axpy_alpha_dram;
    ...
    snrt_dma_start_2d(x, axpy_X_dram, 2 * sizeof(double), 4 * sizeof(double), 2 * sizeof(double), 4);
    // snrt_dma_start_2d(y, axpy_Y_dram,  * sizeof(double), 2 * sizeof(double), 1 * sizeof(double), 1 * sizeof(double));
    snrt_dma_wait_all();
  }
  ...
  // Call AXPY
  faxpy_db_v64b(*a, x_int, y_int, dim_core);
