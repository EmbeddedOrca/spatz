...
def emit_axpy_layer(name="axpy", **kwargs):
    ...
    if dtype != "char":
        layer_str += (
            f'static {dtype} {name}_X_dram [{m}] __attribute__((section(".data"))) = '
            + array_to_cstr(vec_X)
            + ";\n\n\n"
        )
        layer_str += (
            f'static {dtype} {name}_Y_dram [{m}] __attribute__((section(".data"))) = '
            + array_to_cstr(vec_Y)
            + ";\n\n\n"
        )
        layer_str += (
            f'static {dtype} {name}_alpha_dram __attribute__((section(".data"))) = '
            + array_to_cstr(alpha)
            + ";\n\n\n"
        )
        layer_str += (
            f"static const {dtype} {name}_GR_dram[{m}] = "
            + array_to_cstr(result)
            + ";\n\n\n"
        )
    else:
        layer_str += (
            f"static {dtype} {name}_X_dram [{m}] = "
            + array_to_cstr(kwargs["bits_X"], fmt="char")
            + ";\n\n\n"
        )
        layer_str += (
            f"static {dtype} {name}_Y_dram [{m}] = "
            + array_to_cstr(kwargs["bits_Y"], fmt="char")
            + ";\n\n\n"
        )
        layer_str += (
            f"static {dtype} {name}_alpha_dram = "
...
def ascending_data_generator(shape, prec):
    if prec == 64:
